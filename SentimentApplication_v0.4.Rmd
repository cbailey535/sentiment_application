...
my_loughran <- loughran_data %>% filter(sentiment %in% c("positive","negative"))
...
words_by_time <- sd1$data(withSelection = TRUE, withFilter = TRUE) %>%
  ...
  ungroup() %>%
  rename(count = n) %>%
  filter(word_total>=30)  # nbr of words used at least x times
...
renderPlot({
  ...
  comparison.cloud(title.colors=c("red","green"),
                   colors = c("red","green"),
