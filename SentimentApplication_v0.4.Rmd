...
review <- readr::read_csv('raw_text_consolidated.csv',
  ...
  filter(!brand %in% c('Consolidated','Frontier')) 
...
my_loughran <- loughran_data %>% filter(sentiment != "superfluous")
...
f113 <- filter_select("period", "Select Week(s):", sd1, ~week, allLevels = FALSE)
...
words_by_time <- sd1$data(withSelection = TRUE, withFilter = TRUE) %>%
  ...
  ungroup() %>%
  rename(count = n) %>%
  filter(word_total>3)  # nbr of words used at least x times
