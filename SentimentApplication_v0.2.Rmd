...
review_ts_chart <- review_copy %>%
  select(created_at, text, week, brand, region, src) %>%
...
### <font size="5">**Word Ratios: ATT v Xfinity**</font>
```{r}
word_ratios <- tidy_tweets %>%
  filter(brand %in% c("ATT","Xfinity")) %>%
  filter(!str_detect(word, "^@")) %>%
  count(word, brand) %>%
  group_by(word) %>%
  filter(sum(n)>=5) %>%
  ...
  mutate(logratio = log(ATT/Xfinity)) %>%
...
wr_1 <- att_xfin_word_ratios %>%
  ...
  coord_flip() +
  ylab("log odds ratio (Xfinity/ATT)") +
  scale_fill_discrete(name = "", labels = c("Xfinity", "ATT")) 
ggplotly(wr_1)
```
...
sentiment_messages_polarity <- tidy_tweets %>%
  ...
  # filter(src != "FaceBook") %>%
  inner_join(my_lexicon, by = "word") %>%
  group_by(rec, text, brand, created_at, src) %>%
